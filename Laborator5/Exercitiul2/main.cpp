...
int const NR_PARR = 10, NR_MERID = 5;
// (3) pasul cu care vom incrementa u, respectiv v
float step_u = (U_MAX - U_MIN) / NR_PARR * 2, step_v = (V_MAX - V_MIN) / NR_MERID * 2;
...
float alpha = 0.0f, beta = 0.0f, dist = 400.0f;
...
void CreateVBO(void)
{
	...
	glm::vec4 Vertices[(NR_PARR + 1) * NR_MERID], Vertices2[(NR_PARR + 1) * NR_MERID];
...
void RenderFunction(void)
{
	...
	Obsx = Refx + dist * cos(beta) * cos(alpha);
